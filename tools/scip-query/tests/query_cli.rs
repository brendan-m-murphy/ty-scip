...
fn exposes_the_minimal_navigation_surface() {
    ...
    assert_eq!(fixture.success(&["find", "Alpha"])["result"]["total"], 2);
    assert_eq!(
        fixture.success(&["at", "pkg/models.py:2:9"])["result"]["items"][0]["symbol"]["symbol"],
        RUN
    );
    assert_eq!(
    ...
    assert_eq!(
        fixture.success(&["definition", "Alpha.run"])["result"]["definitions"]["total"],
        1
    );
    assert_eq!(
    ...
    assert_eq!(
        fixture.success(&["members", "Alpha"])["result"]["items"][0]["symbol"]["display_name"],
        "run"
    );
    assert_eq!(
        fixture.success(&["supertypes", "Alpha"])["result"]["items"][0]["target"]["symbol"],
        BASE
    );
    assert_eq!(
        fixture.success(&["subtypes", "Base"])["result"]["items"][0]["source"]["symbol"],
        ALPHA
    );
}
...
fn uses_fingerprinted_callee_evidence_for_both_directions() {
    ...
    assert_eq!(callees["result"]["items"][0]["callee"]["symbol"], HELPER);
    assert_eq!(
        callees["result"]["items"][0]["provenance"],
        "ty_scip_callee_position"
    );

    let callers = fixture.success(&["callers", "Alpha.run"]);
    assert_eq!(callers["result"]["total"], 1);
    assert_eq!(callers["result"]["items"][0]["caller"]["symbol"], TEST);
