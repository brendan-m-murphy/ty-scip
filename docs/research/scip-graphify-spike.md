...
The converter preserves document and symbol identities, document-qualified
local symbols, occurrence ranges and roles, smallest-enclosing-definition
ownership, descriptor hierarchy, and explicit SCIP relationships. It emits
references from the enclosing dependent symbol to the target. Standard SCIP
implementation relationships become Graphify `implements` edges; they are not
assumed to mean class inheritance. A plain reference is never described as a
runtime call.

On the frozen 281-document OpenGHG index used for the spike, conversion took
about 0.8 seconds and produced 21,528 nodes and 98,933 occurrence-level edges
in about 65 MB of JSON. Graphify 0.9.55 loaded the result for `path`,
...
same symbols become parallel edges. The spike contained about 60,500 distinct
...
Facts requiring ty or Ruff belong in `ty-scip` and should use standard SCIP
fields where possible. The downstream tool owns only SCIP-to-graph mapping,
serialization, caching, and traversal. An optional producer sidecar is worth
considering only after a useful fact cannot be represented in SCIP and a
measured query need justifies it.
